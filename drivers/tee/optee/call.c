...
/* Requires the corresponding binary session semaphore to be held */
static int verify_ocall_request(u32 num_params, struct optee_call_ctx *call_ctx)
{
	...
	case OPTEE_MSG_RPC_CMD_SHM_FREE:
		...
		/*
		 * This parameter either carries the requested allocation size
		 * or a pointer to the SHM to be freed.
		 */
		if (!arg->num_params ||
			      ...
			      u32 num_params, struct optee_call_ctx *call_ctx)
{
	...
	case TEE_IOCTL_OCALL_CMD_SHM_ALLOC:
		if (call_ctx->rpc_arg->cmd != OPTEE_MSG_RPC_CMD_SHM_ALLOC ||
		    !num_params)
			return -EINVAL;

		/* This parameter carries the allocated SHM ID */
		if (params[0].attr != TEE_IOCTL_PARAM_ATTR_TYPE_VALUE_INOUT ||
		    params[0].u.value.c > INT_MAX)
			return -EINVAL;
		break;
	case TEE_IOCTL_OCALL_CMD_SHM_FREE:
		if (call_ctx->rpc_arg->cmd != OPTEE_MSG_RPC_CMD_SHM_FREE ||
		    !num_params)
			return -EINVAL;

		/* Sanity check, not used while processing the reply */
		if (params[0].attr != TEE_IOCTL_PARAM_ATTR_TYPE_VALUE_INPUT)
			return -EINVAL;
		break;
...
/*
 * Requires the corresponding binary session semaphore to be held (if OCALLs are
 * expected)
 */
				 ...
				 struct optee_call_ctx *call_ctx)
{
	...
	case OPTEE_MSG_RPC_CMD_SHM_FREE:
		...
		params[0].u.value.c = tee_shm_get_id(shm);
		break;
	case OPTEE_MSG_RPC_CMD_OCALL:
...
static int process_ocall_reply(u32 ret, u32 ret_origin,
			       ...
			       struct optee_call_ctx *call_ctx)
{
	const u64 func = tee_param_get_ocall_func(ocall);
	struct tee_shm *shm;
	void *shm_pages_list;
	struct optee_msg_param *msg_param;
	u32 msg_num_params;
	...
	case TEE_IOCTL_OCALL_CMD_SHM_ALLOC:
		...
		shm = tee_shm_get_from_id(call_ctx->ctx,
					  (int)params[0].u.value.c);
		if (IS_ERR(shm)) {
			rc = PTR_ERR(shm);
			goto exit_set_ret;
		}
		...
		break;
	case TEE_IOCTL_OCALL_CMD_SHM_FREE:
		/* OP-TEE ignores ret and ret_origin for an SHM_FREE RPC */
		if (ret != TEEC_SUCCESS)
			goto exit_propagate_ret;
		/* NOTHING */
		break;
...
exit_propagate_ret:
	call_ctx->rpc_arg->ret = ret;
	call_ctx->rpc_arg->ret_origin = ret_origin;
	return -EINVAL;
exit_set_ret:
	call_ctx->rpc_arg->ret = TEEC_ERROR_BAD_PARAMETERS;
	call_ctx->rpc_arg->ret_origin = TEEC_ORIGIN_COMMS;
...
static inline void clear_call_ctx(struct optee_call_ctx *call_ctx)
{
...
/*
 * Requires the corresponding binary session semaphore to be held; the caller is
		       ...
		       struct tee_param *ocall_param)
{
	...
	if (rc == -EAGAIN) {
		...
		tee_param_clear_ocall(ocall_param);
	}
...
{
	sess->call_ctx.rpc_arg->ret = TEEC_ERROR_TARGET_DEAD;
	sess->call_ctx.rpc_arg->ret_origin = TEEC_ORIGIN_COMMS;

	/*
	 * If the session was opened successfully in spite of the OCALL having
	 * been cancelled, close it. An OCALL cancellation is not a normal
	 * execution path and as such the sanity of the CA cannot be guaranteed.
	 */
	if (cancel_ocall(&sess->call_ctx) == 0 &&
	    sess->call_ctx.msg_arg->ret == TEEC_SUCCESS)
		close_session(sess->call_ctx.ctx,
			      sess->call_ctx.msg_arg->session);

	/*
	 * Decrease the ref count on all shared memory pointers passed into the
	 * original function invocation.
	 */
	process_ocall_memrefs(sess->call_ctx.msg_arg->params,
			      sess->call_ctx.msg_arg->num_params, false);

	tee_shm_free(sess->call_ctx.msg_shm);
	kfree(sess);
}
